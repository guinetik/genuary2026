import{G as f,W as u,C as m}from"./index-D3AXu9BF.js";import{CONFIG as i}from"./day31.config-DDGTRykT.js";import{BirdFlock as p}from"./day31.birds-BeLwQsgh.js";import{FlyingPoster as v}from"./day31.posters-4E5DhKD6.js";import{Runner as g}from"./day31.runner-C2T7jUCh.js";import{MatrixRain as y}from"./day31.matrix-B7FGu3RC.js";const w=`precision highp float;\r
\r
...
vec2 intersect(vec3 ro, vec3 rd) {\r
    float d = 0.0, h = 0.0;\r
\r
    // Early-out: looking up from above terrain = sky\r
    if (rd.y > 0.1 && ro.y > 2.0) return vec2(-1);\r
\r
    for (int i = 0; i < 300; i++) {\r
        vec3 p = ro + d * rd;\r
\r
        // Aggressive skip when above terrain\r
        if (p.y > 4.0) {\r
            if (rd.y > 0.0) return vec2(-1);\r
            d += (p.y - 4.0) * 0.5;\r
            continue;\r
        }\r
\r
        ...
        h = s.x;\r
\r
        // Adaptive step: bigger when far from surface\r
        float stepMult = h > 1.0 ? 0.7 : 0.4;\r
        d += h * stepMult;\r
\r
        // Fixed threshold (não cresce com distância = sem fantasmas)\r
        if (abs(h) < 0.002)\r
            return vec2(d, s.y);\r
        if (d > 150.0) break;\r
    }\r
...
\r
    ...
    // Optimized: 150 iterations with adaptive stepping (was 200 fixed)\r
...
\r
        // Adaptive step: smaller near event horizon, larger far away\r
        float adaptiveStep = stepSize * mix(0.8, 1.5, smoothstep(0.2, 1.5, dist));\r
\r
...
\r
        ...
        float distToHorizon = dist - eventHorizonRadius;\r
        notCaptured = smoothstep(0.0, 0.666, distToHorizon);\r
\r
        // Early exit if fully captured\r
...
\r
        ...
        // Reuse dist instead of recalculating length(toBH)\r
        float distFromBH = dist - eventHorizonRadius;\r
        vec3 diskColor = mix(innerDiskColor, outerDiskColor, pow(max(distFromBH, 0.0), 2.0));\r
...
`;class S extends f{constructor(r){super(r),this.backgroundColor="#000"}init(){if(super.init(),this.renderScale=i.renderScale||1,this.scaledWidth=Math.floor(this.width*this.renderScale),this.scaledHeight=Math.floor(this.height*this.renderScale),this.webgl=new u(this.scaledWidth,this.scaledHeight),!this.webgl.isAvailable()){console.warn("WebGL not available"),this.useFallback=!0;return}this.webgl.useProgram("finale",w,b),this.webglBH=new u(this.scaledWidth,this.scaledHeight),this.webglBH.useProgram("blackhole",k,x),this.bhPosition=[.5,.55],this.bhSize=.04,this.time=0,this.completion=0,this.cameraZ=0,this.mouseX=0,this.mouseY=0,this.mouseActive=0,this.camera=new m({perspective:i.perspective,rotationX:0,rotationY:0}),this.posters=[],this.loadPosters(),this.spawnCooldown=.5,this.nextPosterIndex=0,this.nextSide=1,this.canvas.addEventListener("mousemove",r=>{const e=this.canvas.getBoundingClientRect();this.mouseX=(r.clientX-e.left)/e.width*2-1,this.mouseY=(r.clientY-e.top)/e.height*2-1,this.mouseActive=1}),this.canvas.addEventListener("mouseleave",()=>{this.mouseActive=0}),this.canvas.addEventListener("touchmove",r=>{r.preventDefault();const e=this.canvas.getBoundingClientRect(),n=r.touches[0];this.mouseX=(n.clientX-e.left)/e.width*2-1,this.mouseY=(n.clientY-e.top)/e.height*2-1,this.mouseActive=1}),this.canvas.addEventListener("touchend",()=>{this.mouseActive=0}),this.paintingTexture=null,this.pulse=0,this.pulseDecay=2.5,this.flocks=[],this.nextFlockTime=3+Math.random()*5,this.runner=new g(this),this._loadRunner(),this.matrixRain=new y,this.matrixRain.init(this.width,this.height)}async _loadRunner(){await this.runner.load()}spawnFlock(){const r=this.width,e=this.height,n=i.birdFlockSize.min+Math.floor(Math.random()*(i.birdFlockSize.max-i.birdFlockSize.min+1)),l=Math.floor(Math.random()*4);let o,a,t,c;const s=e*.1,h=e*.45;switch(l){case 0:o=-50,a=s+Math.random()*(h-s),t=r+50,c=s+Math.random()*(h-s);break;case 1:o=r+50,a=s+Math.random()*(h-s),t=-50,c=s+Math.random()*(h-s);break;case 2:o=-50,a=s,t=r+50,c=h;break;default:o=r+50,a=s,t=-50,c=h}this.flocks.push(new p(o,a,t,c,n))}triggerPulse(){this.pulse=1}loadPosters(){for(let r=1;r<=i.posterCount;r++){const e=new Image,n=String(r).padStart(3,"0")+".jpg";e.src=`/${n}`;const l=new v(e,r-1);e.onload=()=>{l.loaded=!0},this.posters.push(l)}}update(r){super.update(r),this.time+=r,this.completion=Math.min(1,this.time/5),this.cameraZ+=i.posterSpeed*r;for(const n of this.posters)n.update(this.cameraZ);const e=this.posters.filter(n=>n.active).length;this.spawnCooldown-=r,this.spawnCooldown<=0&&e<i.maxVisible&&(this.spawnNextPoster(),this.spawnCooldown=i.spawnDelay),this.mouseActive>0?(this.camera.rotationY=this.mouseX*.2,this.camera.rotationX=this.mouseY*.08):(this.camera.rotationY*=.95,this.camera.rotationX*=.95),this.pulse>0&&(this.pulse=Math.max(0,this.pulse-this.pulseDecay*r));for(const n of this.flocks)n.update(r,this.width,this.height);this.flocks=this.flocks.filter(n=>n.active),this.nextFlockTime-=r,this.nextFlockTime<=0&&(this.spawnFlock(),this.nextFlockTime=i.birdFlockInterval.min+Math.random()*(i.birdFlockInterval.max-i.birdFlockInterval.min)),this.runner&&this.runner.update(r),this.matrixRain&&this.matrixRain.update(r)}spawnNextPoster(){let r=null;for(let e=0;e<this.posters.length;e++){const n=(this.nextPosterIndex+e)%this.posters.length;if(!this.posters[n].active&&this.posters[n].loaded){r=this.posters[n],this.nextPosterIndex=(n+1)%this.posters.length;break}}r&&(r.spawn(this.cameraZ,this.nextSide),this.nextSide*=-1,this.triggerPulse())}render(){const r=this.ctx,e=this.width,n=this.height,l=this.scaledWidth,o=this.scaledHeight;if(this.useFallback){r.fillStyle="#000",r.fillRect(0,0,e,n);return}this.webgl.setUniforms({uTime:this.time,uResolution:[l,o],uMouse:[this.mouseX,this.mouseY],uMouseActive:this.mouseActive,uCompletion:this.completion,uHasTexture:this.paintingTexture?1:0,uPulse:this.pulse}),this.webgl.clear(0,0,0,1),this.webgl.render(),r.imageSmoothingEnabled=!0,r.drawImage(this.webgl.getCanvas(),0,0,e,n);const a=this.bhPosition[0]-this.mouseX*this.mouseActive*.35,t=this.bhPosition[1]-this.mouseY*this.mouseActive*.03;this.webglBH.setUniforms({uTime:this.time,uResolution:[l,o],uMouse:[this.mouseX,this.mouseY],uMouseActive:this.mouseActive,uPulse:this.pulse,uBHPosition:[a,t],uBHSize:this.bhSize}),this.webglBH.clear(0,0,0,0),this.webglBH.render(),r.drawImage(this.webglBH.getCanvas(),0,0,e,n);for(const c of this.flocks)c.render(r);this.runner&&this.runner.render(r,e,n),this.renderPosters(r,e,n),this.matrixRain&&this.matrixRain.render(r,e,n)}renderPosters(r,e,n){const l=e/2,o=n/2,a=this.posters.filter(t=>t.active);a.sort((t,c)=>c.z-t.z);for(const t of a)t.render(r,this.cameraZ,l,o,n)}onResize(){this.scaledWidth=Math.floor(this.width*this.renderScale),this.scaledHeight=Math.floor(this.height*this.renderScale),this.webgl&&this.webgl.isAvailable()&&this.webgl.resize(this.scaledWidth,this.scaledHeight),this.webglBH&&this.webglBH.isAvailable()&&this.webglBH.resize(this.scaledWidth,this.scaledHeight)}stop(){this.webgl&&this.webgl.destroy(),this.webglBH&&this.webglBH.destroy(),super.stop()}}function z(d){const r=new S(d);return r.start(),{stop:()=>r.stop(),game:r}}export{z as default};
