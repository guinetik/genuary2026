...
class FactoredNetwork {
  ...
  backward(target, cache) {
    ...
    for (let i = 0; i < this.embedSize; i++) {
      let sum = 0;
      for (let j = 0; j < this.nTokens; j++) {
        sum += dLogits[j] * this.embed[j][i];
        // Accumulate embed gradient from unembed path
        this.grad_embed[j][i] += dLogits[j] * out[i];
      }
      dOut[i] = sum;
    }
